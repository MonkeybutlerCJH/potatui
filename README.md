...
- **Terrestrial weather** — live weather conditions, forecast, and severe weather alerts from the US National Weather Service. Shows current temperature and conditions as a pill in the header bar. Flashes red and fires a toast when an Extreme or Severe weather alert (tornado, severe thunderstorm, flash flood) is issued for your park's location. Click to see full current conditions (temperature, humidity, wind), a 7-day forecast, and active alert details. Polls every 10 minutes; gracefully handles non-US locations and offline stations.
...
- **Weather indicator** — shows current temperature and a weather emoji (e.g. `72° ⛅`) in the header bar. Flashes red when an Extreme or Severe weather alert (tornado, severe thunderstorm, flash flood warning) is active at your park's location and fires a toast notification. Click to open a detail panel with current conditions (temperature, humidity, wind), a 7-day forecast, and full alert text with severity levels. Uses the free US National Weather Service API — no authentication required. Polls every 10 minutes; skipped in offline mode.
